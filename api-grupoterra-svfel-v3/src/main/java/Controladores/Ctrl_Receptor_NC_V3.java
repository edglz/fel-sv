...
import Entidades.Receptor_nc;
import java.io.Serializable;
import java.sql.Connection;
import java.sql.Statement;

public class Ctrl_Receptor_NC_V3 implements Serializable {
    ...
    public Receptor_nc obtener_receptor_nc_v3(Long id_dte, Connection conn) {
        ...
        try {
            ...
            resultado.setNit(ctrl_base_datos.ObtenerString("SELECT F.NIT FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            resultado.setNrc(ctrl_base_datos.ObtenerString("SELECT F.NRC FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            resultado.setNombre(ctrl_base_datos.ObtenerString("SELECT F.NOMBRE FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            resultado.setCodActividad(ctrl_base_datos.ObtenerString("SELECT C.CODIGO FROM CAT_019 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_019 FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte + ")", conn));
            resultado.setDescActividad(ctrl_base_datos.ObtenerString("SELECT C.VALOR FROM CAT_019 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_019 FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte + ")", conn));
            resultado.setNombreComercial(ctrl_base_datos.ObtenerString("SELECT F.NOMBRECOMERCIAL FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            
            Direccion direccion_nc = new Direccion();
            direccion_nc.setDepartamento(ctrl_base_datos.ObtenerString("SELECT C.CODIGO FROM CAT_012 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_012 FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte + ")", conn));
            direccion_nc.setMunicipio(ctrl_base_datos.ObtenerString("SELECT C.CODIGO FROM CAT_013 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_013 FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte + ")", conn));
            direccion_nc.setComplemento(ctrl_base_datos.ObtenerString("SELECT F.DIRECCION_COMPLEMENTO FROM RECEPTOR_NC_V3 F WHERE F.ID_DTE=" + id_dte, conn));
    ...
    public String extraer_receptor_jde_nc_v3(Long id_dte, String ambiente, String AN8_JDE, String SHAN_JDE, Connection conn) {
        String resultado = "";
        ...
        try {
            ...
            String cadenasql = "INSERT INTO RECEPTOR_NC_V3 ("
                    + "ID_DTE, "
                    + "ID_RECEPTOR, "
                    + "NIT, "
                    + "NRC, "
            ...
            cadenasql = "INSERT INTO SHIPTO_NC_V3 ("
                    + "ID_DTE, "
                    + "ID_SHIPTO, "
                    + "NIT, "
                    + "NRC, "
            ...
            System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:extraer_receptor_jde_nc_v3()|ERROR:" + ex.toString());
        }
