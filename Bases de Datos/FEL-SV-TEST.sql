...
SELECT DISTINCT F.SDKCOO, F.SDDOCO, F.SDDCTO, F.SDDOC, F.SDDCT, F.SDMCU, F.SDAN8, F.SDSHAN, F.SDCRCD, F.SDIVD, '000' STCD, '-' CRSREF01, '-' CRSREF02, '-' CRSREF03, '-' CRSREF04, '-' CRSREF05, 'F42119' TABLA, F.SDCRMD
FROM CRPDTA.F42119@JDEPY F 
WHERE 
...
-- ((F.SDDOCO,F.SDDCTO) NOT IN (SELECT G.FEDOCO, G.FEDCTO FROM CRPDTA.F5542FEL@JDEPY G)) AND
(F.SDIVD = 12200);
