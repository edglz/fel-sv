    ...
    public List<CuerpoDocumento_f> obtener_cuerpo_documento_f_v3(Long id_dte, Connection conn) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                cuerpo_documento_f.setPrecioUni(ctrl_base_datos.ObtenerDouble("SELECT A.PRECIOUNI + (A.PRECIOUNI * (B.VALOR/A.VENTAGRAVADA)) VALOR FROM CUERPO_DOCU_CCF_V3 A LEFT JOIN CUERPO_TRIBUTO_CCF_V3 B ON (A.ID_DTE=B.ID_DTE AND A.ID_CUERPO_DOCUMENTO=B.ID_CUERPO_DOCUMENTO AND B.ID_CAT_015=1) WHERE A.ID_DTE=" + id_dte + " AND A.ID_CUERPO_DOCUMENTO=" + id_cuerpo_documento, conn));
                cuerpo_documento_f.setMontoDescu(ctrl_base_datos.ObtenerDouble("SELECT A.MONTODESCU + (A.MONTODESCU * (B.VALOR/A.VENTAGRAVADA)) VALOR FROM CUERPO_DOCU_CCF_V3 A LEFT JOIN CUERPO_TRIBUTO_CCF_V3 B ON (A.ID_DTE=B.ID_DTE AND A.ID_CUERPO_DOCUMENTO=B.ID_CUERPO_DOCUMENTO AND B.ID_CAT_015=1) WHERE A.ID_DTE=" + id_dte + " AND A.ID_CUERPO_DOCUMENTO=" + id_cuerpo_documento, conn));
                ...
                cuerpo_documento_f.setVentaGravada(ctrl_base_datos.ObtenerDouble("SELECT A.VENTAGRAVADA + (A.VENTAGRAVADA * (B.VALOR/A.VENTAGRAVADA)) VALOR FROM CUERPO_DOCU_CCF_V3 A LEFT JOIN CUERPO_TRIBUTO_CCF_V3 B ON (A.ID_DTE=B.ID_DTE AND A.ID_CUERPO_DOCUMENTO=B.ID_CUERPO_DOCUMENTO AND B.ID_CAT_015=1) WHERE A.ID_DTE=" + id_dte + " AND A.ID_CUERPO_DOCUMENTO=" + id_cuerpo_documento, conn));
