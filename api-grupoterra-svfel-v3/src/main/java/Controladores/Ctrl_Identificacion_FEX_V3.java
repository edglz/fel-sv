...
import Entidades.Identificacion_fex;
import java.io.Serializable;
import java.sql.Connection;
import java.sql.Statement;
import java.util.UUID;

public class Ctrl_Identificacion_FEX_V3 implements Serializable {

    private static final long serialVersionUID = 1L;

    public Ctrl_Identificacion_FEX_V3() {
    }

    public Identificacion_fex obtener_identificacion_fex_v3(Long id_dte, Connection conn) {
        Identificacion_fex resultado = new Identificacion_fex();

        try {
            Ctrl_Base_Datos ctrl_base_datos = new Ctrl_Base_Datos();
            
            resultado.setVersion(ctrl_base_datos.ObtenerEntero("SELECT F.DTE_VERSION FROM IDENTIFICACION_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            resultado.setAmbiente(ctrl_base_datos.ObtenerString("SELECT C.CODIGO FROM CAT_001 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_001 FROM IDENTIFICACION_FEX_V3 F WHERE F.ID_DTE=" + id_dte + ")", conn));
            resultado.setTipoDte(ctrl_base_datos.ObtenerString("SELECT C.CODIGO FROM CAT_002 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_002 FROM IDENTIFICACION_FEX_V3 F WHERE F.ID_DTE=" + id_dte + ")", conn));
            resultado.setNumeroControl(ctrl_base_datos.ObtenerString("SELECT F.NUMEROCONTROL FROM IDENTIFICACION_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            resultado.setCodigoGeneracion(ctrl_base_datos.ObtenerString("SELECT F.CODIGOGENERACION FROM IDENTIFICACION_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn));
            ...
            System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:obtener_identificacion_fex_v3()|ERROR:" + ex.toString());
        }

        return resultado;
    }
    
    public String extraer_identificacion_jde_fex_v3(Long id_dte, String ambiente, String DCTO_JDE, String MCU_JDE, String CRCD_JDE, String IVD_JDE, Connection conn) {
        String resultado = "";
        ...
        try {
            ...
            String cadenasql = "UPDATE EMISOR_ESTABLECIMIENTO_V3 SET CORRELATIVO_FEX=CORRELATIVO_FEX+1 WHERE CODPUNTOVENTA='" + MCU_JDE + "'";
            ...
            cadenasql = "INSERT INTO IDENTIFICACION_FEX_V3 ("
                    + "ID_DTE,"
                    + "ID_IDENTIFICACION, "
                    + "DTE_VERSION, "
                    + "ID_CAT_001, "
            ...
            System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:extraer_identificacion_jde_fex_v3()|ERROR:" + ex.toString());
        }
