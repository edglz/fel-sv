...
public class Ctrl_Resumen_F_V3 implements Serializable {
    ...
    public String extraer_resumen_jde_f_v3(Long id_dte, String ambiente, Connection conn) {
        ...
        try {
            ...
            Number TOTALGRAVADA = ctrl_base_datos.ObtenerDouble("SELECT SUM(A.VENTAGRAVADA + (A.VENTAGRAVADA * (B.VALOR/A.VENTAGRAVADA))) VALOR FROM CUERPO_DOCU_CCF_V3 A LEFT JOIN CUERPO_TRIBUTO_CCF_V3 B ON (A.ID_DTE=B.ID_DTE AND A.ID_CUERPO_DOCUMENTO=B.ID_CUERPO_DOCUMENTO AND B.ID_CAT_015=1) WHERE A.ID_DTE=" + ID_DTE, conn);
            ...
            Number TOTALDESCU = ctrl_base_datos.ObtenerDouble("SELECT SUM(A.MONTODESCU + (A.MONTODESCU * (B.VALOR/A.VENTAGRAVADA))) VALOR FROM CUERPO_DOCU_CCF_V3 A LEFT JOIN CUERPO_TRIBUTO_CCF_V3 B ON (A.ID_DTE=B.ID_DTE AND A.ID_CUERPO_DOCUMENTO=B.ID_CUERPO_DOCUMENTO AND B.ID_CAT_015=1) WHERE A.ID_DTE=" + ID_DTE, conn);
