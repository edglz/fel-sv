...
public class Ctrl_DTE_FEX_V3 implements Serializable {

    private static final long serialVersionUID = 1L;

    public Ctrl_DTE_FEX_V3() {
    }

    public List<Long> extraer_documento_jde_fex_v3(String ambiente) {
        ...
        try {
            ...
            while (rs.next()) {
                Long ID_DTE = ctrl_base_datos.ObtenerLong("SELECT NVL(MAX(F.ID_DTE), 0) + 1 MAXIMO FROM DTE_FEX_V3 F", conn);
                ...
                cadenasql = "INSERT INTO DTE_FEX_V3 ("
                        + "ID_DTE, "
                        + "KCOO_JDE, "
                        + "MCU_JDE, "
                        + "DOCO_JDE, "
                ...
                Ctrl_CuerpoDocumento_FEX_V3 ctrl_cuerpo_documento_fex_v3 = new Ctrl_CuerpoDocumento_FEX_V3();
                String result_cuerpo_documento = ctrl_cuerpo_documento_fex_v3.extraer_cuerpo_documento_jde_fex_v3(ID_DTE, ambiente, KCOO_JDE.trim(), DOCO_JDE.trim(), DCTO_JDE.trim(), JEVER_JDE.trim(), conn);

                Ctrl_Resumen_FEX_V3 ctrl_resumen_fex_v3 = new Ctrl_Resumen_FEX_V3();
                String result_resumen = ctrl_resumen_fex_v3.extraer_resumen_jde_fex_v3(ID_DTE, ambiente, conn);
            ...
            try {
                ...
                System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:extraer_documento_jde_fex_v3()-rollback|ERROR:" + ex1.toString());
                resultado.clear();
            }
            ...
            try {
                ...
                System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:extraer_documento_jde_fex_v3()-finally|ERROR:" + ex.toString());
                resultado.clear();
            }
        }
    ...
    public DTE_FEX_V3 generar_json_dte_fex_v3(String ambiente, Long id_dte) {
        DTE_FEX_V3 resultado = new DTE_FEX_V3();
        Connection conn = null;
        ...
        try {
            ...
            try {
                ...
                System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:generar_dte_fex_v3()|ERROR:" + ex.toString());;
            } catch (Exception ex1) {
                System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:generar_dte_fex_v3()-rollback|ERROR:" + ex.toString());
            }
        } finally {
            try {
                if (conn != null) {
                ...
                System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:generar_dte_fex_v3()-finally|ERROR:" + ex.toString());
            }
        }
    ...
    public void registro_db_respuesta_mh(String ambiente, RESPUESTA_RECEPCIONDTE_MH respuesta_recepciondte_mh, Long id_dte) {
        ...
        try {
            ...
            String cadenasql = "UPDATE DTE_FEX_V3 SET "
            ...
            String NUMEROCONTROL = ctrl_base_datos.ObtenerString("SELECT F.NUMEROCONTROL FROM IDENTIFICACION_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String KCOO_JDE = ctrl_base_datos.ObtenerString("SELECT F.KCOO_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String DOCO_JDE = ctrl_base_datos.ObtenerString("SELECT F.DOCO_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String DCTO_JDE = ctrl_base_datos.ObtenerString("SELECT F.DCTO_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String DOC_JDE = ctrl_base_datos.ObtenerString("SELECT F.DOC_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String DCT_JDE = ctrl_base_datos.ObtenerString("SELECT F.DCT_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String AEXP_JDE = ctrl_base_datos.ObtenerString("SELECT REPLACE(TO_CHAR(F.TOTALPAGAR,'9999999999D99MI'),'.','') AEXP_JDE FROM RESUMEN_FEX_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            ...
            if (respuesta_recepciondte_mh.getCodigoMsg().trim().equals("001") || respuesta_recepciondte_mh.getCodigoMsg().trim().equals("002")) {
                ...
                File TargetFileJson = new File("/FELSV3/json/jsondte_fex_" + id_dte + ".json");
                Adjunto adjunto_json = new Adjunto();
                adjunto_json.setName("jsondte_fex_" + id_dte + ".json");
                adjunto_json.setType("application/json");
                InputStream inputstream_mail_json = new FileInputStream(TargetFileJson);
                byte[] bytes_json = IOUtils.toByteArray(inputstream_mail_json);
                adjunto_json.setData(Base64.getEncoder().encodeToString(bytes_json));
