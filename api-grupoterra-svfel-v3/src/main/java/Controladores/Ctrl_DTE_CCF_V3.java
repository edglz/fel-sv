    ...
    public List<Long> extraer_documento_jde_ccf_v3(String ambiente) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String result_identificacion = ctrl_identificacion_ccf_v3.extraer_identificacion_jde_ccf_v3(ID_DTE, ambiente, DCTO_JDE.trim(), MCU_JDE.trim(), CRCD_JDE.trim(), IVD_JDE.trim(), conn);

                Ctrl_Receptor_CCF_V3 ctrl_receptor_ccf_v3 = new Ctrl_Receptor_CCF_V3();
                String result_recepor = ctrl_receptor_ccf_v3.extraer_receptor_jde_ccf_v3(ID_DTE, ambiente, AN8_JDE.trim(), SHAN_JDE.trim(), DOCO_JDE.trim(), DCTO_JDE.trim(), conn);
    ...
    public void registro_db_respuesta_mh(String ambiente, RESPUESTA_RECEPCIONDTE_MH respuesta_recepciondte_mh, Long id_dte) {
        ...
        try {
            ...
            if (respuesta_recepciondte_mh.getCodigoMsg().trim().equals("001") || respuesta_recepciondte_mh.getCodigoMsg().trim().equals("002")) {
                ...
                File TargetFileJson = new File("/FELSV3/json/jsondte_ccf_" + id_dte + ".json");
                ...
                String MCU_JDE = ctrl_base_datos.ObtenerString("SELECT F.MCU_JDE FROM DTE_CCF_V3 F WHERE F.ID_DTE=" + id_dte, conn);
