...
import Entidades.DocumentoRelacionado_nd;
...
public class Ctrl_DocumentoRelacionando_ND_V3 implements Serializable {

    private static final long serialVersionUID = 1L;

    public Ctrl_DocumentoRelacionando_ND_V3() {
    }

    public List<DocumentoRelacionado_nd> obtener_documento_relacionado_nd_v3(Long id_dte, Connection conn) {
        List<DocumentoRelacionado_nd> resultado = new ArrayList<>();
        DocumentoRelacionado_nd documento_relacionado_nd = new DocumentoRelacionado_nd();

        try {
            Ctrl_Base_Datos ctrl_base_datos = new Ctrl_Base_Datos();

            documento_relacionado_nd.setTipoDocumento(ctrl_base_datos.ObtenerString("SELECT C.CODIGO FROM CAT_002 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_002 FROM DOCU_RELA_ND_V3 F WHERE F.ID_DOCUMENTOS_RELACIONADOS=1 AND F.ID_DTE=" + id_dte + ")", conn));
            documento_relacionado_nd.setTipoGeneracion(ctrl_base_datos.ObtenerEntero("SELECT C.CODIGO FROM CAT_007 C WHERE C.ID_CAT IN (SELECT F.ID_CAT_007 FROM DOCU_RELA_ND_V3 F WHERE F.ID_DOCUMENTOS_RELACIONADOS=1 AND F.ID_DTE=" + id_dte + ")", conn));
            documento_relacionado_nd.setNumeroDocumento(ctrl_base_datos.ObtenerString("SELECT F.NUMERODOCUMENTO FROM DOCU_RELA_ND_V3 F WHERE F.ID_DOCUMENTOS_RELACIONADOS=1 AND F.ID_DTE=" + id_dte, conn));
            documento_relacionado_nd.setFechaEmision(ctrl_base_datos.ObtenerString("SELECT TO_CHAR(F.FECHAEMISION,'YYYY-MM-DD') FROM DOCU_RELA_ND_V3 F WHERE F.ID_DOCUMENTOS_RELACIONADOS=1 AND F.ID_DTE=" + id_dte, conn));
            resultado.add(documento_relacionado_nd);
        } catch (Exception ex) {
            System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:obtener_documento_relacionado_nd_v3()|ERROR:" + ex.toString());
        }

        return resultado;
    }

    public String extraer_documento_relacionado_jde_nd_v3(Long id_dte, String ambiente, String JEVER_JDE, String KCOO_JDE, String DOCO_JDE, String DCTO_JDE, Connection conn) {
        String resultado = "";
        ...
        try {
            ...
            if (documento_relacionado != null) {
                ...
                if (partes_documento_relacionado.length == 2) {
                    ...
                    String cadenasql = "INSERT INTO DOCU_RELA_ND_V3 ("
                            + "ID_DTE,"
            ...
            resultado = "";
            System.out.println("PROYECTO:api-grupoterra-svfel-v3|CLASE:" + this.getClass().getName() + "|METODO:extraer_documento_relacionado_jde_nd_v3()|ERROR:" + ex.toString());
        }
