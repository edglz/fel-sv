...
public class Ctrl_Apendice_ND_V3 implements Serializable {
    ...
    public List<Apendice_nd> obtener_apendice_nd_v3(Long id_dte, Connection conn) {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                String valor = ctrl_base_datos.ObtenerString("SELECT F.VALOR FROM APENDICE_ND_V3 F WHERE F.ID_DTE=" + id_dte + " AND F.ID_APENDICE=" + id_apendice, conn);
                if(valor.length() > 150) {
                    valor = valor.substring(0, 149);
                }
                apendice_nd.setValor(valor);
    ...
    public String extraer_apendice_jde_nd_v3(Long id_dte, String ambiente, String DOCO_JDE, String DCTO_JDE, String MCU_JDE, Connection conn) {
        ...
        try {
            ...
            String AN8_JDE = ctrl_base_datos.ObtenerString("SELECT F.AN8_JDE FROM DTE_ND_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
            ...
            cadenasql = "INSERT INTO APENDICE_ND_V3 ("
                    + "ID_DTE, "
                    + "ID_APENDICE, "
                    + "CAMPO, "
                    + "ETIQUETA, "
