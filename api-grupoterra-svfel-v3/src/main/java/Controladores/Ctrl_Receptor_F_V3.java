...
public class Ctrl_Receptor_F_V3 implements Serializable {
    ...
    public String extraer_receptor_jde_f_v3(Long id_dte, String ambiente, String AN8_JDE, String SHAN_JDE, Connection conn) {
        String resultado = "";
        ...
        try {
            ...
            String DIRECCION_COMPLEMENTO = ctrl_base_datos.ObtenerString("SELECT NVL(TRIM(NVL(TRIM(F.ALADD2),' ') || ' ' || NVL(TRIM(F.ALADD3),' ')),'Código de cliente sin registro de dirección.') DIRECCION FROM " + esquema + ".F0116@" + dblink + " F WHERE ROWNUM=1 AND F.ALAN8=" + AN8_JDE, conn);
            ...
            String DIRECCION_COMPLEMENTO_SHAN = ctrl_base_datos.ObtenerString("SELECT NVL(TRIM(NVL(TRIM(F.ALADD2),' ') || ' ' || NVL(TRIM(F.ALADD3),' ')),'Código de cliente sin registro de dirección.') DIRECCION FROM " + esquema + ".F0116@" + dblink + " F WHERE ROWNUM=1 AND F.ALAN8=" + SHAN_JDE, conn);
